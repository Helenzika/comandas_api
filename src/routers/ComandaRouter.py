...
async def get_comanda(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
	try:
...
async def create_comanda(comanda_data: ComandaCreate, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
	try:
...
@limiter.limit("restrictive")
async def update_comanda(id: int, comanda_data: ComandaUpdate, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
	try:
...
async def delete_comanda(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
	try:
		# Buscar a comanda
		result = await db.execute(select(ComandaDB).where(ComandaDB.id == id))
		comanda = result.scalar_one_or_none()
...
@limiter.limit("restrictive")
async def add_produto_to_comanda(comanda_id: int, produto_data: ComandaProdutosCreate, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth =
...
@limiter.limit("moderate")
async def get_comanda_produtos(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
	try:
		# Verificar se a comanda existe
		result = await db.execute(select(ComandaDB).where(ComandaDB.id == id))
		comanda = result.scalar_one_or_none()
		...
		# Construir response manualmente
		produtos_response = []
		for comanda_produto, produto, funcionario in produtos_result:
...
@limiter.limit("restrictive")
async def update_comanda_produto(id: int, produto_data: ComandaProdutosUpdate, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth =
...
@limiter.limit("critical")
async def remove_produto_from_comanda(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
	try:
